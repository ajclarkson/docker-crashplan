...
TARGETDIR=/var/crashplan

# Populate the TARGETDIR
rsync -a --ignore-existing ${SOURCEDIR}/ ${TARGETDIR}/

# Update install paths
sed -i -r "s@${SOURCEDIR}@${TARGETDIR}@p" /etc/init.d/crashplan
sed -i -r "s@^TARGETDIR=.*@TARGETDIR=${TARGETDIR}@g" ${TARGETDIR}/install.vars
...
_link ${TARGETDIR}/id /var/lib/crashplan
